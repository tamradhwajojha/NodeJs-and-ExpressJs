...
exports.postForgotPassword = async (req, res, next) => {
  ...
  try {
    ...
    const forgotEmail = {
      to: email,
      from: 'schrodingersmileyy@gmail.com',
...
exports.postSignup = [
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      // Try to send welcome email, but do not block signup on failure
      try {
        await sendGrid.send(welcomeEmail);
      } catch (emailError) {
        console.error('Failed to send welcome email:', emailError && emailError.message ? emailError.message : emailError);
      }

      return res.redirect("/login");
    } catch (err) {
      // Handle duplicate email error from MongoDB gracefully
      const errorMessage = (err && err.code === 11000) ? 'Email already exists' : err.message;
      return res.status(422).render('auth/signup', 
        {
          pageTitle: 'Login', 
          isLoggedIn: false,
